	...
	def fit(self, labelledTweets, labelledTweetScores, unlabelledTweets):
		...
		with timer("Using reinforcement learning"):
			with timer("Generating Constants"):
				numberOfTrainingIterations = 100
				m = 500
			...
			while(numberOfTrainingIterations > 0):
				...
				print("\nIteration : ", 200 - numberOfTrainingIterations)
				with timer("Making label vector"):
					Y = np.zeros([len(labelledTweetScores), 5], dtype=int)
					for x in range(0, len(labelledTweetScores)):
							Y[x, labelledTweetScores[x]] = 1
...
with timer("Starting cross validation"):
	kf = KFold(n_splits = 10)
	gAccu = 0
	gPrecision = [0, 0, 0, 0, 0]
	gRecall =  [0, 0, 0, 0, 0]
	gFScore =  [0, 0, 0, 0, 0]
	gSupport =  [0, 0, 0, 0, 0]
	Features = X
...
with timer("Cross Validation"):
	...
	for train_index, test_index in kf.split(Features):
		iteration += 1
		with timer("Cross Validation for iteration : " + str(iteration)):
			X_Train = [Features[x] for x in train_index]
			X_Test = [Features[x] for x in test_index]
			labelTrain = [label[x] for x in train_index]
			labelTest = [label[x] for x in test_index]

			model = RLAN()
			model.fit(X_Train, labelTrain, unlabelledTweets)

			with timer("Making test label vector"):
				testLabelVector = np.zeros([len(labelTest), 5], dtype=int)
				for x in range(0, len(labelTest)):
						testLabelVector[x, labelTest[x]] = 1

			with timer("Making X_Test Vector"):
				tokenisedTest = tokenizer.texts_to_sequences(X_Test)
				X_Test = sequence.pad_sequences(tokenisedTest, maxlen=180,padding='post')

			with timer("Calculating scores "):
				accuracy = model.predictorModel.evaluate(X_Test,labelTest,verbose = 100)[1]
				predictTest = model.predictorModel.predict_classes(X_Test, verbose = 1)
				prec1, recall1, fscore1, support1 = precision_recall_fscore_support(labelTest, predictTest)
				...
				for i in range(0, len(prec1)):
					gPrecision[i] += prec1[i]
					gRecall[i] += recall1[I]
					gFScore[i] += fscore1[i]
					gSupport[i] += support1[i]
...
with timer("final Output"):
	...
	gSupport = [x / 10 for x in gSupport]
