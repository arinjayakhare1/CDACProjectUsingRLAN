...
with timer("making Tokeniser"):
    ...
    relevantIter = 0
    gRelevantRadicalAccu = 0
    gRelevantPrecision = [0,0]
    gRelevantRecall = [0,0]
    gRelevantFScore = [0,0]
    gRelevantEpochs = 0
    ...
    gRelevantPositivePredRadical = 0
...
with timer("Cross Validation"):
    for train_index, test_index in kf.split(Features):
        with timer("Making nueral network for iteration : " + str(iteration + 1)):
            ...
            print("\nRadical Training Done for Iteration ",iteration, "with epochs : ", epochs)
            ...
            prec, recall, fscore, support = precision_recall_fscore_support(radicalTest1, predictRadical)
            ...
            if(accuRadicalLstm > 0.8):
                gRelevantRadicalAccu += accuRadicalLstm
                gRelevantPositivePredRadical += len(positivePredRadical)
                gRelevantPrecision[0] += prec[0]
                gRelevantPrecision[1] += prec[1]
                gRelevantRecall[0] += recall[0]
                gRelevantRecall[1] += recall[1]
                gRelevantFScore[0] += fscore[0]
                gRelevantFScore[1] += fscore[1]
                relevantIter += 1
                gRelevantEpochs += epochs

            
            print("\n\nRelevant Iterations till now : ", relevantIter)    
...
with timer("final Output"):
    ...
    gRelevantRadicalAccu /= relevantIter
    gRelevantPrecision = [x / relevantIter for x in gRelevantPrecision]
    gRelevantRecall = [x / relevantIter for x in gRelevantRecall]
    gRelevantFScore = [x / relevantIter for x in gRelevantFScore]
    ...
    print("Average epochs : ", (gEpochs / 10))


    print("\n\n\n\nRelevant Iterations : ", relevantIter)       


    print("\n\n\n\nRelevant scores for LSTM : ","\nRadical : ",gRelevantRadicalAccu)
    print("Precision : ", gRelevantPrecision)
    print("Recal : ", gRelevantRecall)
    print("FScore : ", gRelevantFScore)
    print("Positive Predicitions in total : ")
    print("Radical : ", gRelevantPositivePredRadical)
    print("Positive Predicitions in average : ")
    print("Radical : ", gRelevantPositivePredRadical / relevantIter) 
    print("Total relevant epochs : ", gRelevantEpochs)
    print("Average epochs : ", (gRelevantEpochs / relevantIter))  
